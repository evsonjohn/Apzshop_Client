    ...
    <section class="py-3 mb-3" style="background: url(../images/background-pattern.jpg);">
      <div class="container-fluid">
        <div class="d-flex justify-content-between">
          <h1 class="page-title pb-0">Order Details</h1>
        </div>
      </div>
    </section>
    <?php $order_id = $_GET['order_info'];
    $order_info = order_info_vendor($order_id);
    ?>
    <div class="container-fluid">
      <div class="container">
        <div class="d-flex justify-content-between align-items-center py-3">
          <h2 class="h5 mb-0">&nbsp;&nbsp;&nbsp;&nbsp;<?php echo "Order ID: ".$order_id; ?></h2>
        </div>
        <?php $cancel_reason = cancel_reason($order_id); ?>
        <span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo "Order_date: ".$order_info['order_date']; ?></span>
        <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Status: <?php 
                        if($order_info['order_status']== 1){echo "Pending"; }
                        elseif($order_info['order_status']== 2){ echo "Order Shipped";}
                        elseif($order_info['order_status']== 3){ echo "On Delivery";}
                        elseif($order_info['order_status']== 4){ echo "Delivered"."<br>";
                                                                    echo "&nbsp;&nbsp;&nbsp;&nbsp;Reason of Cancelation: ".$cancel_reason['reason'];}//cancelation order
                        elseif($order_info['order_status']== 5){ echo "Cancel Order";}
                        elseif($order_info['order_status']== 6){ echo "Order Received"."<br>";
                                                                    echo "&nbsp;&nbsp;&nbsp;&nbsp; Reason of Cancelation: ".$cancel_reason['reason']; }//cancelation order
                        elseif($order_info['order_status']== 7){ echo "Pending cancel";
                        echo "<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Reason: ".$cancel_reason['reason']."</p>";}  ?></p>
            ...
            <div class="card mb-4">
              <div class="card-body">
                <div class="mb-3 d-flex justify-content-between">
                  <div>
                    <span class="me-3"><?php
                    //estimated time of delvery
                    $connection = new connection();
                    $conn = $connection->con();
                    $stmt = "SELECT `order_date` FROM `vendor_order` WHERE `order_id` = '$order_id'"; //the value of 1 is pending
                    $result = $conn->prepare($stmt);
                    $result->execute();
                    
                    if ($result->rowCount() > 0) {
                        // Fetch the start date from the first row
                        $row = $result->fetch(PDO::FETCH_ASSOC);
                        $startDate = $row["order_date"]; // Assuming the column name is "order_date" in your table
                        ...
                        $estimatedDeliveryDate = calculateEstimatedDelivery($startDate, $days);
                    
                          echo '<div style="background-color: #f0f0f0; padding: 10px; border-radius: 5px;">';
                          echo '<span style="font-size: 18px;"><b>Estimated delivery date:</b> ' . $estimatedDeliveryDate . '</span>';
                          echo '</div>';
                      ...
                      <td>
                        <div class="d-flex mb-2">
                          <div class="flex-shrink-0">
                            <?php $prd_img = products_img($order_info['product_id']);   ?>
                            <img src="../images/product_pictures/<?php echo $prd_img['1st_img']; ?>" alt="product_picture" width="35" class="img-fluid">
                          </div>
                          <div class="flex-lg-grow-1 ms-3">
                            <h6 class="small mb-0"><a href="#" class="text-reset"><?php echo $order_info['product_name'];  ?></a></h6>
                            <?php $prd_info = prd_info($order_info['product_id']);
                            if($prd_info['product_category'] == 1 || $prd_info['product_category'] == 2){ ?>
                            <span class="small">Color: <?php echo $order_info['option_1']; ?></span>
                            <br>
                            <span class="small">Size: <?php echo $order_info['option_2']; ?></span>
                            <?php }?>
                        </div>
                      </td>
                      <td><?php echo "Quantity: ".$order_info['product_qty']; ?></td>
                      <td class="text-end"><?php echo "₱".$order_info['price']; ?></td>
                    </tr>
                  </tbody>
                  <tfoot>
                    <tr>
                    ...
                    <tr>
                      ...
                      <td class="text-end"><?php echo "₱".$order_info['shipping_fee']; ?></td>
                    </tr>

                    <tr class="fw-bold">
                      <td colspan="2">TOTAL</td>
                      <?php $total = $order_info['shipping_fee'] + $order_info['price'];?>
                      <td class="text-end"><?php echo "₱".$order_info['price']; ?></td>
                    </tr>
                  </tfoot>
                </table>
              ...
              <div class="card-body">
                <div class="row">
                  <div class="col-lg-6">
                    ...
                    <p><?php if($order_info['payment_method']== 1){
                      echo "Cash on Delivery (COD)";
                    }else{
                      echo "Paypal";
                    } ?><br>
                    Total: <?php echo "₱".$order_info['price']." ";   ?><span class="badge bg-success rounded-pill">Pending</span></p>

                  </div>
                  <div class="col-lg-6">
                    <h3 class="h6">Billing address</h3>
                    <address>
                      <strong><?php echo $order_info['buyer_name']; ?></strong><br>
                      <?php $address = address($order_info['buyer_add']);?>
                      <?php echo $address['street']." ".$address['barangay'];?>
                      <br>
                      <?php echo $address['city']." ".$address['province'];?>
                      <br>
                      <?php echo "Zip code: ".$address['zip_code'];?>
                      <br>
                      <?php $user_data = user_data($order_info['buyer_id']);  ?>
                      <abbr title="Phone">Phone number: (+63)<?php echo " ".$order_info['phone_number'];?></abbr>
                    </address>
                  </div>
                </div>
              </div>
          ...
          <div class="col-lg-4">
            ...
            <div class="card mb-4"> -->
              ...
              <div class="card-body">
                ...
                <strong></strong>
                <span><a href="#" class="line  text-decoration-underline" target="_blank"></a> <i class="bi bi-box-arrow-up-right"></i> </span>
                <hr>
